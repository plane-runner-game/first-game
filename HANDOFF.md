...
- Camera (`CameraFollow.cs`): rig base position (0, 4.55, −11), pitch 13.5° (2026-09-17, third pass: "the plane I move still disappears under my thumb, I want 40% of the screen height for the thumb", then "lower it a bit, 35%" — the squad now sits at 42% of the screen height at altitude 0 and 57% at the ceiling (where every attempt starts, section 3.2), the front crate at ~66%, horizon at ~75%. History: (0, 6.2, −9.5) pitch 8° put the squad at 11–28%; pitch 17° put it at 29–45% with the horizon at 82%, "too little sky"; (0, 5, −9.5) pitch 11° kept 29–46% with the horizon at 70%, still under the thumb; y 3.95 gave 47–63%, "a bit too high"), FOV 52. **Dive framing** (2026-09-18, "when I go down for the crates the camera goes down so far with me that the planes above no longer show"): blended by altitude / `altitudeMax`, the rig sits `dollyLow` 4 further back (z −15) and the camera looks `pitchLow` 8.9° instead of `pitchHigh` 13.5° at altitude 0, so the swarm stays at 65–75% of the screen (it was at 78–86%, under the top HUD and in the horizon haze) while the squad keeps its 42%; at the ceiling nothing changes. Everything is ~27% smaller at altitude 0 (distance 15 vs 11) — a zoom-out on the dive. The rig
...
- **Weapon crates** (`CrateDef.weapon`, `BreakableKind.Weapon`): the next `WeaponDef` up from the last
  ...
  of the crate** (`Breakable.showcase`: its `planePrefab` at 1.5× on the box at 1.5, turning slowly, under **the weapon crate's own parachute** — `Breakable.weaponCrateMesh` = `MeshFactory.CrateWeapon()`: the same box under a bigger, taller canopy with gores striped weapon-colour / white (submeshes 2 / 3, the 4th material is a white instance of the canopy), six cords, a scalloped skirt and a knob on top (requested 2026-09-16 after two misreads: "a distinctive shape — it has a parachute, but a distinctive one"; a glow quad behind the plane was tried and cancelled: "a rectangle around it") —
